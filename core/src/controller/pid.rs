...
impl PidController {
    ...
    pub fn default_quad() -> Self {
        Self {
            ...
            // ⚠️ **暂为 0，待联合整定后再翻**（2026-09-21 实测）：
            // 轨迹跟踪上 ki_v_xy=1.0 最优（滞后 7.741m→1.440m ✓），
            // 但**位置阶跃**（pos_ctrl::outer_position_step）上会引入**尾部残留振荡**：
            //   settle=未稳定、尾部峰值 0.230m（±5% 带 = 0.15m）✗ —— 而稳态误差仅 -0.045m ✓
            // ⇒ 不是"判据按旧律标的"（该判据合法 ✓），而是**速度积分引入轻阻尼慢模态** ✗
            // ⇒ 属**联合整定**问题（跟踪 vs 阶跃），归阶段 7；可能需更小值或改进抗饱和。
            ki_v_xy: 0.0,
