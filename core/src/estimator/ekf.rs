...
/// 此值则线性抑制到 0 ✓）。
pub static mut G_AW_RAWGATE: f32 = 0.3;
...
#[used]
// ✅ **B 阶段交付：默认 1.0**（2026-09-21）—— 配合 `G_AW_RAWGATE=0.3`（单侧 ✓）
// 与 `G_AW_MAG_GATE`（编译期 0.05 ✓），**全量 fly-sim-core 145 + flyctrl-core 105
// 连续三次零失败** ✓（无顺序依赖 ✓）。
//
// 收益（realistic 源，行为量）：A3 急刹 −6.0% ✓ · A7 慢转+0.5g −21.1% ✓ ·
// A1 悬停 / A2 巡航 持平 ✓ · **A9 自由落体【精确抑制】** ✓（= 补偿关档，逐位相同；
// 未加门时 83.4° ✗✗）。
//
// ⚠️ 转正路上修正过的【两个我自己引入的设计错误】（留作记录）：
//  ① 门限曾取 0.10 ✗（全开点 0.40g）—— 那是基于**0.5g 单点扫描**的结论，
//     覆盖不足 ⇒ 0.2g 测例收益崩塌（≥50% → 23% ✗）。**正解是 0.05** ✓。
//  ② 原始比力门首版用**对称**偏差 ||f|/g − 1| ✗ ⇒ 误伤【大水平加速度】（0.8g 刹车
//     ⇒ |f|≈1.28g）这一**合法**情形（此时加速度计仍正确测重力方向）⇒ 改为**单侧**
//     （只抑制 |f|/g < 1 = 自由落体侧 ✓；过大侧交给既有幅值门 ✓）。
//
// ⚠️ 历史：本静态曾因"源质量"（多普勒噪声/延迟）被置回 0.0；本次转正建立在
//    "原始比力门（非循环 ✓）+ 幅值门下限可整定"两项修复之上 ✓。
pub static mut G_AW_GPS: f32 = 1.0;
...
impl EkfEstimator {
    ...
    pub fn new(
        ...
        EkfEstimator {
            ...
            // ⚠️ **att_alpha 的定案（2026-09-21）**：由 SIL 侧 10min 零噪声探针证明
            // —— 重力锚定**把估计钉在比力方向、而真值在漂** ⇒ **解耦估计与真值**，
            // 使控制环对真实漂移失明（`sil` 的末窗口姿态误差 RMS 3.55° vs 前窗口 0.17°）。
            // 实测：**越强越差**（零噪声姿态摆幅 0.11°→6.53°→17.08° 对应 0/0.02/0.08）。
            // 而它的"收益"（压陀螺零偏漂移）**已无判据依赖**（SIL 全回归在 att_alpha=0
            // 下 300/0 全绿）—— 因为 F7 硬铁同步 + 磁输入同源 + 风场统一这三处
            // 输入等价性修复，已把驱动姿态漂移的主因拿掉。
            // ⇒ 本参数**归零**；能力保留在 `G_ATT_ALPHA` 旋钮（默认 -1 = 用本值）。
            // 调用方若显式传值（如 SIL 的 `new(0.0, …)`）不受影响。
            att_alpha,
    ...
    pub fn default_quad() -> Self {
        ...
        // 不随本项改善，另行处理。
        // ⚠️ **att_alpha = 0.02 保持不变**（2026-09-21 实测教训）：
        // 一度把它归零以"同步"到 SIL 的 `FlyController`（后者用 0.0），结果
        // **打破 7 项 `att_est`** —— 因为锚定是**场景依赖**的：
        //   - `att_est`（量**估计误差**）：锚定压住陀螺漂移 ⇒ **需要**；
        //   - 闭环测试（量**真值**）：锚定**解耦估计与真值** ⇒ 有害（`sil`/`mag_hover`）。
        // ⇒ SIL 现在的"分裂"（`FlyController::new(0.0, …)` vs `att_est` 自带参数）
        // 本来就是正确设计；**本默认值服务于估计类测试，不应为闭环结论而拉平**。
        // 固件（闭环）是否该用 0，应由 **M 场自己的测量**决定，而非从 SIL 抄一个值。
                ...
                || self.world_accel[2] != 0.0
            {
                ...
                let raw_gate = unsafe {
                    ...
                    if th <= 0.0 {
                        ...
                        // ⚠️ **单侧**（2026-09-21 修正）：只抑制【|f| 过小】= 自由落体侧 ✓，
                        // **不**抑制 |f| 过大 ✗ —— 后者是"大水平加速度"（如 A3 0.8g 刹车 ⇒
                        // |f|≈1.28g），加速度计【仍正确测量重力方向】⇒ 补偿应照常生效 ✓。
                        // 首版用对称偏差 ||f|/g − 1| ⇒ 误伤该情形（实测 A3 0.8g 收益从
                        // ≥50% 掉到 26% ✗）。过大侧的门由既有幅值门（|a| 超 2.5g 衰减）负责 ✓。
                        let an_raw =
                            sqrt(a[0] * a[0] + a[1] * a[1] + a[2] * a[2]) / g;
                        let dev = (1.0 - an_raw).max(0.0); // 单侧：只在 an_raw < 1 时生效 ✓
...
impl EkfEstimator {
    ...
    fn update_mag(&mut self, mag: Option<[f32; 3]>) {
        ...
        // ---- 路线 2.3：以当前估计为预测的**磁新息门**（方向类）—— 实测【否决】2026-09-21 ----
        // 曾实现 `G_MAG_YAW_GATE_DEG`：隐含航向修正超门限则拒绝本次锚定。
        // 实测（未标定·极端硬铁 + 偏航扫掠，门限 60/45/30/20°）：
        //   基线 max=179.966° ；60°->179.966° ；45°->179.944° ；30°->179.997° ；20°->160.991°（RMSE 反而更差 139°）
        //   ⇒ **门完全不触发** ✗。根因（结构性，与模长门同源）：
        //   **渐进拖拽时估计值一直紧跟被扰动的磁** ⇒ 每拍隐含修正只有几度 ⇒ 门永不触发；
        //   等它变大时估计已被拖走。⇒ **以当前估计为预测的新息门，对"被跟随的扰动"看不见** ✗。
        // 且陀螺对照也救不了：恒定机体系硬铁的**速率特征与真值一致**（偏置随机体旋转），
        //   只有**绝对偏置**错 ⇒ 陀螺只能验速率 ⇒ **恒定硬铁在飞行中原理上不可检测** ✓。
        // ⇒ 唯一修法是**离线标定扣除**（与 F7 结论一致 ✓）。故本门已回退，不留未证实代码 ✗。
