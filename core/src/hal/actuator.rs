...
///
/// 注：曾用 IEEE-754 位模式比较（`v.to_bits() < 0.0f32.to_bits()`）绕开 mcu_simulater
/// 的 ARMv7 VFP 模拟缺陷。但位模式比较对**负数**语义错误（负数的符号位使其位模式
/// 大于所有正数，负值会绕过下界变成 1.0），在 host SIL/phy 仿真下导致悬停翻滚发散。
/// 已恢复标准浮点 clamp；若 mcu_simulater VFP 缺陷仍存在，应在模拟器层修复而非改语义。
