...
| 里程碑 | 内容 | 状态 | 验收 |
|--------|------|------|------|
| **M1** | 基线闭环验证（单位/状态机/估计算法/控制律/物理/闭环悬停） | ✅ 完成 | 悬停闭环稳定收敛（修复 mixer+物理符号 bug） |
| **M2** | 算法横向对比基线（EKF/LQR + 5 场景 CLI） | ✅ 完成 | `--est/--ctrl/--scenario` 一键对比表 |
| **M3** | 控制深度（MPC/轨迹前馈/机型配置/FDIR 雏形） | ✅ 完成 | `--ctrl mpc`、`--fdir` 演示 |
| **M4** | 仿真真实度（精细气动/故障注入/蒙特卡洛/实时因子） | ✅ 完成 | `--fault/--robust/--montecarlo/--rtf` |
| **M5** | HAL 与嵌入式落地（sensor/actuator/rtos 占位 + 零堆审计） | ✅ 完成 | `--features stm32f407` 编译过、no_alloc 集成测试 |
| **M6** | 通信与地面站（MAVLink/内部链路/遥测节流） | ✅ 完成 | `--comm` 600 ATT/POS 帧 0 丢、CRC 全过 |
| **M7** | 形式化与验证（不变量/类型级安全网/HIL 桥接） | ✅ 完成 | 37 项属性+集成测试绿 |
| **M8** | 算法先进性（INDI/学习估计/多机编队） | ✅ 完成 | 44 项测试绿、`--swarm` 双机 V 编队收敛 |
| **M9** | 任务层与飞行模式（航点/围栏/模式治理） | ✅ 完成 | 51 项测试绿、`--mission` 4 航点到达 |
...
M10 收尾（A：stm32f407 全特性回归 + release 构建 + 进度清单）已于 2026-08-08 完成并核查通过。
后续可选两条主线：

- **B（中等风险）：真实跨节点发布/订阅发现协议**。把 `#4` 的 `Registry` 从"板内静态 + 运行时补登"
  升级为"组件启动时经链路广播 `announce` 帧、对端 `SwarmTable`/总线索引入邻居组件表"，实现多机
  拓扑自动汇聚（当前 `Registry` 仅板内可见，跨机尚不可见）。
- **C（最高价值、脱离 SIL）：真实 F407 HAL 驱动落地**。在 `core/src/hal/stm32f407` 的占位模块上接
  真实外设：`UartLink`/`UsbCdcLink`（MAVLink/neighbor 链路）、`PwmActuator`/`DshotActuator`
  （执行器）、`SpiImu`/`I2cMag`/`UartGps`（传感器），使 `--bus`/`--swarm-link` 能在真实 F407 上跑。
  前置：自研 Rust RTOS 或裸机周期循环 + `irq.rs` 的 `BasepriLock` 已就绪。
